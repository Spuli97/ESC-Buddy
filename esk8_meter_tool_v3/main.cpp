   ...
   if(ask('n', 'y'))
    ...
    else
    {
        ...
        if(ask('n', 'y'))
        {
            ...
            scanf("%lf", &diameter);
            printf("Please enter your gear ratio. If the motor spins faster than the wheel this number is >0 and <1. It is 1.0 for hub or direct drive motors. You can enter decimal digits, use the '.' as decimal point. Confirm with ENTER (not SPACE):\n");
            scanf("%lf", &gear_ratio);

            printf("Please enter your motor pole-pair count (ERPM per RPM value). Confirm with ENTER (not SPACE):\n");
            scanf("%lf", &erpm_per_rpm);
            spd = ((float)(getERPM_per_KMPH(diameter, erpm_per_rpm))) / ((float)gear_ratio);

            printf("Please enter your VESC's tachometer steps per motor revolution. Confirm with ENTER (not SPACE):\n");
            scanf("%lf", &tac_steps);

            tac = ((float)(getTAC_per_KM(diameter, tac_steps))) / ((float)gear_ratio);
        }
    }
    printf("\n");
